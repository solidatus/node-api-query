...
const { QUERY_ERROR_NAMES } = require('./queryErrors')
...
const SPECIAL_PROPERTIES = {
  ...
  Entity: {
    ...
    $transitions(e) {
      incoming = _.map(e.incoming, t => getTransitionObject(t, true))
      outgoing = _.map(e.outgoing, t => getTransitionObject(t, false))
      return {
        isTransitionsCollection: true,
  ...
  Transition: {
    ...
    $source: e => e.source,
    $target: e => e.target,
    $transitions: () => ({ isTransitionsCollection: true, items: [] }),
    $incoming: () => [],
    $outgoing: () => [],
    $objects: () => [],
    $attributes: () => [],
  ...
  Transitions: {
    deep(ts) {
      if (this.entity === null || !this.entity.isEntity) {
        return []
      }
      const incoming = _.map(this.entity.getIncoming(), t => getTransitionObject(t, true))
      const outgoing = _.map(this.entity.getOutgoing(), t => getTransitionObject(t, false))
      return incoming.concat(outgoing)
    },
    incoming(ts) {
      return _.filter(ts.items, t => t.isIncoming)
    },
    outgoing(ts) {
      return _.filter(ts.items, t => t.isOutgoing)
    }
  }
}
...
class EvaluationContext {
  ...
  resolveProperty(entity, property) {
    ...
    const type =
      ...
      (entity.isTransitionsCollection && 'Transitions') ||
...
const evaluate = {
  ...
  ALL(node, context) {
    ...
    if (!_.isArray(list) && !_.isFunction(list.next) || true) {
...
const getTransitionObject = (t, isIncoming) => {
  const transitionObject = {}
  _.assign(transitionObject, t)
  transitionObject.isIncoming = isIncoming
  transitionObject.isOutgoing = !isIncoming
  transitionObject.isTransition = true
